...
namespace PSNC.Multimedia
{
    ...
    public enum MPEG_MODE
    {
        ...
        JOINTSTEREO = 0x0002,
        DUALCHANNEL = 0x0004,
        SINGLECHANNEL = 0x0008
    }


    internal class FmtChunk
    {
        bool extended;
        int _samplesInBlock = 1;
        short wFormatTag;
        ushort wChannels;
        ...
        {
            get { return (WaveFormatTag)wFormatTag; }
            set { wFormatTag = (short)value; }
        }
        ...
        public ulong SamplesPerSec
        {
            ...
            set { dwSamplesPerSec = value; }
        }

        public ulong AvgBytesPerSec
        {
            get { return dwAvgBytesPerSec; }
            set { dwAvgBytesPerSec = value; }
        }

        public ushort BlockAlign
        {
            get { return wBlockAlign; }
            set { wBlockAlign = value; }
        }
        ...
        public MPEG_LAYER HeadLayer
        {
            get { return (MPEG_LAYER)fwHeadLayer; }
            set { fwHeadLayer = (ushort)value; }
        }


        public ulong HeadBitrate
        {
            get { return dwHeadBitrate; }
        }

        public MPEG_MODE HeadMode
        {
            get { return (MPEG_MODE)fwHeadMode; }
            set { fwHeadMode = (ushort)value; }
        }


        public int SamplesInBlock
        {
            get { return _samplesInBlock; }
            set { _samplesInBlock = value; }
        }
        ...
        public void Parse(MediaStreamReader br, uint chunkSize)
        {
            wFormatTag = br.ReadInt16();
            wChannels = br.ReadUInt16();
            dwSamplesPerSec = br.ReadUInt32();
            dwAvgBytesPerSec = br.ReadUInt32();
            wBlockAlign = br.ReadUInt16();
            wBitsPerSample = br.ReadUInt16();

            if (chunkSize > 16)
            {
                extended = true;
                _samplesInBlock = 144 * 8;
                wBitsPerSample = 16;

                ushort cbSize = br.ReadUInt16();
                if (cbSize >= 22)
                {
                    fwHeadLayer = br.ReadUInt16();
                    dwHeadBitrate = br.ReadUInt32();
                    fwHeadMode = br.ReadUInt16();
                    fwHeadModeExt = br.ReadUInt16();
                    wHeadEmphasis = br.ReadUInt16();
                    fwHeadFlags = br.ReadUInt16();
                    dwPTSLow = br.ReadUInt32();
                    dwPTSHigh = br.ReadUInt32();
        ...
        public override string ToString()
        {
            ...
            sb.Append(string.Format("\n\tFormat:\t\t{0} ", this.FormatTag));
            sb.Append(string.Format("\n\tChannels:\t{0} ", this.Channels));
            sb.Append(string.Format("\n\tSamplesPerSec:\t{0} ", this.SamplesPerSec));
            sb.Append(string.Format("\n\tAvgBitsPerSec:\t{0} ", this.AvgBytesPerSec * 8));
            sb.Append(string.Format("\n\tBlockAlign:\t{0} ", this.BlockAlign));
            sb.Append(string.Format("\n\tBitsPerSample:\t{0} ", this.BitsPerSample));

            if (extended)
            {
                sb.Append(string.Format("\n\tMpeg Layer:\t{0} ", this.HeadLayer));
                sb.Append(string.Format("\n\tHead Bitrate:\t{0} ", this.HeadBitrate));
                sb.Append(string.Format("\n\tHead Mode:\t{0} ", this.HeadMode));
            }
            return sb.ToString();
        }
    }
