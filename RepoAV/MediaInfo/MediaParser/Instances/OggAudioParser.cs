...
{
    public class OggAudioParser : AbstractParser, IMediaParserInstance
    {
        ...
        xml.AudioCoding.Values _coding = xml.AudioCoding.Values.OGG;

        MediaParser.FileFormat fileFormat = MediaParser.FileFormat.OGG;
        AudioStreamProperties _audiostream = null;
        VideoStreamProperties _videostream = null;

        string _mime = "audio/ogg";
        ...
        public bool Test(PSNC.Multimedia.Tools.MediaStreamReader br)
        {
            ...
            using (var media = MediaInfoWrapper.MediaInfoFactory.GetInstance())
            {
                ...
                var format = media.Get<String>(Generalinfo.Format);
                result = format.ToLowerInvariant().Contains("TODO");
            }
            return result;
        }
        ...
        string IMediaParserInstance.ToString()
        {
            ...
            sb.Append(string.Format("FileFormat: {0}", MediaParser.FileFormat.OGG));
        ...
        public string ToXML(PSNC.Multimedia.Tools.DictionaryEx<string, object> dict)
        {
            ...
            _coding = XmlTools.AudioCoding.Values.OGG;
            xml.AddElement(codec, XmlTools.Coding, _coding.EnumToString());
            xml.AddElement(codec, XmlTools.Sample.Name, XmlTools.Sample.FindClosestValue((int)_audiostream.BitsPerSample));
            xml.AddElement(codec, XmlTools.SampleRate.Name, XmlTools.SampleRate.FindClosestValue(_audiostream.SamplesPerSec));
        ...
        {
            string codec = "Vorbis";
            _audiostream = new AudioStreamProperties(0, (int)(this._bitrate), (int)_samplerate, (int)_bits_per_sample, WaveFormatTag.UNKNOWN, MPEG_LAYER.Unknown, false, codec);
        }
